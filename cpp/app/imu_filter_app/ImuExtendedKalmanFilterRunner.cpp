...
ImuExtendedKalmanFilterRunner::~ImuExtendedKalmanFilterRunner()
{
...
void ImuExtendedKalmanFilterRunner::calculateCalibratedAcceleration(const gsl_vector *lg, gsl_vector *a_calibrated) const
{
...
void ImuExtendedKalmanFilterRunner::calculateCalibratedAngularRate(const gsl_vector *lw, gsl_vector *w_calibrated) const
{
...
bool ImuExtendedKalmanFilterRunner::loadCalibrationParam(const std::string &filename)
{
...
bool ImuExtendedKalmanFilterRunner::readAdisData(gsl_vector *accel, gsl_vector *gyro) const
{
...
void ImuExtendedKalmanFilterRunner::initializeGravity(const size_t Ninitial)
{
	...
	for (size_t i = 0; i < Ninitial; ++i)
	{
		...
		ImuExtendedKalmanFilterRunner::readAdisData(measuredAccel_, measuredAngularVel_);
#endif
...
void ImuExtendedKalmanFilterRunner::initializeGravity(const size_t Ninitial, const std::vector<Acceleration> &accels, const std::vector<Gyro> &gyros)
{
	double accel_x_sum = 0.0, accel_y_sum = 0.0, accel_z_sum = 0.0;
	//double gyro_x_sum = 0.0, gyro_y_sum = 0.0, gyro_z_sum = 0.0;
...
bool ImuExtendedKalmanFilterRunner::testAdisUsbz(const size_t loopCount)
{
	size_t loop = 0;
...
/*static*/ bool ImuExtendedKalmanFilterRunner::loadSavedImuData(const std::string &filename, const size_t &Nsample, std::vector<Acceleration> &accels, std::vector<Gyro> &gyros)
{
	std::ifstream stream(filename.c_str());
