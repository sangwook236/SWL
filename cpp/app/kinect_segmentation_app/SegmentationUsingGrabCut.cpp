...
void run_grabcut_using_structure_tensor_mask(const cv::Mat &rgb_input_image, const cv::Mat &structure_tensor_mask)
{
	...
	//grabCut_mask.setTo(cv::Scalar::all(cv::GC_PR_BGD));
	grabCut_mask.setTo(cv::Scalar::all(cv::GC_PR_FGD));
	grabCut_mask.setTo(cv::Scalar::all(cv::GC_FGD), SWL_FGD == structure_tensor_mask);  // foreground.
	grabCut_mask.setTo(cv::Scalar::all(cv::GC_BGD), SWL_BGD == structure_tensor_mask);  // background.
...
#if 0
	// show foreground & background masks.
	//cv::imshow("foreground mask", SWL_FGD == structure_tensor_mask);  // foreground.
	//cv::imshow("background mask", SWL_BGD == structure_tensor_mask);  // background.
