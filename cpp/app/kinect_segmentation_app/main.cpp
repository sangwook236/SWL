...
#elif 1
		// construct depth-guided map using morphological operation of depth boundary.
		construct_depth_guided_map_using_morphological_operation_of_depth_boundary(valid_depth_image, depth_validity_mask, depth_guided_map);
#endif
...
void segment_foreground_based_on_structure_tensor()
{
	...
	for (std::size_t i = 0; i < num_images; ++i)
	{
		...
		if (!load_structure_tensor_mask(structure_tesor_mask_file_list[i], structure_tensor_mask))
			continue;
#elif 0
