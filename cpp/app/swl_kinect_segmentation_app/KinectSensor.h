#if !defined(__SWL_KINECT_SEGMENTATION_APP__KINECT_SENSOR__H_)
#define __SWL_KINECT_SEGMENTATION_APP__KINECT_SENSOR__H_ 1
...
{
public:
	KinectSensor(const cv::Size &imageSize_ir, const cv::Size &imageSize_rgb);
	~KinectSensor();

private:
	explicit KinectSensor(const KinectSensor &rhs);
...
private:
	...
	void rectifyImagePairUsingOpenCV(const cv::Mat &ir_input_image, const cv::Mat &rgb_input_image, cv::Mat &ir_output_image, cv::Mat &rgb_output_image) const;
	...
	void rectifyImagesPairFromLeftToRightUsingOpenCV(const cv::Mat &input_image_left, const cv::Mat &input_image_right, cv::Mat &output_image_left, cv::Mat &output_image_right) const;
	void undistortImagePairUsingOpenCV(const cv::Mat &input_image, cv::Mat &output_image, const cv::Size &imageSize, const cv::Mat &K, const cv::Mat &distCoeffs, const cv::Mat rmap[2]) const;
...
private:
	...
	cv::Mat K_ir_, K_rgb_;
	cv::Mat distCoeffs_ir_, distCoeffs_rgb_;
	cv::Mat R_, T_;
...
#endif  // __SWL_KINECT_SEGMENTATION_APP__KINECT_SENSOR__H_
