...
public:
	...
	void runForwardAlgorithm(const size_t N, const uivector_type &observations, dvector_type &scale, dmatrix_type &alpha, double &logProbability) const;
	// backward algorithm without scaling.
	void runBackwardAlgorithm(const size_t N, const uivector_type &observations, dmatrix_type &beta) const;
	// backward algorithm with scaling.
	void runBackwardAlgorithm(const size_t N, const uivector_type &observations, const dvector_type &scale, dmatrix_type &beta) const;
	...
	bool trainByML(const size_t N, const uivector_type &observations, const double terminationTolerance, const size_t maxIteration, size_t &numIteration, double &initLogLikelihood, double &finalLogLikelihood);
	//	-. for multiple independent observation sequences.
	bool trainByML(const std::vector<size_t> &Ns, const std::vector<uivector_type> &observationSequences, const double terminationTolerance, const size_t maxIteration, size_t &numIteration, std::vector<double> &initLogLikelihoods, std::vector<double> &finalLogLikelihoods);

	// MAP learning using conjugate prior.
	//	-. for a single independent observation sequence.
	bool trainByMAPUsingConjugatePrior(const size_t N, const uivector_type &observations, const double terminationTolerance, const size_t maxIteration, size_t &numIteration, double &initLogLikelihood, double &finalLogLikelihood);
	//	-. for multiple independent observation sequences.
	bool trainByMAPUsingConjugatePrior(const std::vector<size_t> &Ns, const std::vector<uivector_type> &observationSequences, const double terminationTolerance, const size_t maxIteration, size_t &numIteration, std::vector<double> &initLogLikelihoods, std::vector<double> &finalLogLikelihoods);

	// MAP learning using entropic prior.
	//	-. for a single independent observation sequence.
	bool trainByMAPUsingEntropicPrior(const size_t N, const uivector_type &observations, const double z, const bool doesTrimParameter, const double terminationTolerance, const size_t maxIteration, size_t &numIteration, double &initLogLikelihood, double &finalLogLikelihood);
	//	-. for multiple independent observation sequences.
...
protected:
	...
	//
	virtual void doComputeObservationLikelihood(const size_t N, const uivector_type &observations, dmatrix_type &obsLikelihood) const;
	virtual void doComputeExpectedSufficientStatistics(const size_t N, const uivector_type &observations, const dmatrix_type &gamma, const std::vector<dmatrix_type> &xi, dvector_type &expNumVisits1, dvector_type &expNumVisitsN, dmatrix_type &expNumTrans, dmatrix_type &expNumEmit) const;
	virtual void doComputeExpectedSufficientStatistics(const std::vector<size_t> &Ns, const std::vector<uivector_type> &observationSequences, const std::vector<dmatrix_type> &gammas, const std::vector<std::vector<dmatrix_type> > &xis, dvector_type &expNumVisits1, dvector_type &expNumVisitsN, dmatrix_type &expNumTrans, dmatrix_type &expNumEmit) const;
	...
	virtual void doEstimateObservationDensityParametersByMAPUsingEntropicPrior(const unsigned int state, const double z, const bool doesTrimParameter, const dmatrix_type &expNumEmit, std::vector<double> &thetaEmit, std::vector<bool> &isObservationsTrimmed) = 0;
