...
%
% This implementation uses lower (but unknown)
% damping to improve damping observability.
%
clear all;close all;
...
disp(' ');
disp('This particular implementation uses relatively');
disp('small (but unknown) damping to improve');
disp('observability of the damping coefficient');
disp(' ');
%
% Model parameters
%
zeta  = 0.02;    % TRUE damping factor (unitless)
...
% Temporal update
%
   x      = Phix*x + delta; % update of true state w/o noise   
%
...
pause
subplot(3,1,1),plot(tm,p11);xlabel('Time (sec)');ylabel('Position');title('Root mean squared estimation uncertainties');
