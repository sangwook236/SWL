...
public:
	...
	///
	T norm() const  {  return (T)std::sqrt(real_*real_ + imag_*imag_);  }
	/// -pi <= angle <= pi
...
template<typename T>
Complex<T> sqrt(const Complex<T> &z, const int iIndex = 0)
...
{
	T tR((T)std::sqrt(z.norm()));
	T tVal((z.arg() + T(MathConstant::_2_PI*iIndex)) / T(2));
	return Complex<T>(tR*(T)std::cos(tVal), tR*(T)std::sin(tVal));
}

template<typename T>
Complex<T> pow(const Complex<T> &z1, const Complex<T>& z2, const int iIndex = 0)
// z1^z2 = exp(z2 * ln(z1))
{  return exp(z2 * ln(z1, iIndex));  }
/*
template<typename T>
Complex<T> pow10(const Complex<T> &z, const int iIndex = 0)
// 10^z = exp(z * ln(10))
...
{  return Complex<T>((T)std::exp(z.real())*(T)std::cos(z.imag()), (T)std::exp(z.real())*(T)std::sin(z.imag()));  }

template<typename T>
Complex<T> ln(const Complex<T> &z, const int iIndex = 0)
...
// where Ln(z) : the principal value of ln(z), = ln(abs(z)) + j * arg(z)
{
	...
	return Complex<T>((T)std::log(z.norm()), z.arg()+T(MathConstant::_2_PI*iIndex));
}

template<typename T>
Complex<T> log(const Complex<T> &z, const int iIndex = 0)
{  return ln(z, iIndex);  }

template<typename T>
Complex<T> log10(const Complex<T> &z, const int iIndex = 0)
{  return logb(T(10), z, iIndex);  }

template<typename T>
Complex<T> logb(const T& base, const Complex<T> &z, const int iIndex = 0)
// obtain k-th non-principal value
// ln(z) = Ln(z) + j * 2 * k * pi (k = ..., -2, -1, 0, 1, 2, ...)
// where Ln(z) : the principal value of ln(z), = ln(abs(z)) + j * arg(z)
{
	...
	return Complex<T>((T)std::log(z.norm())/tVal, (z.arg()+T(MathConstant::_2_PI*iIndex))/tVal);
}

template<typename T>
Complex<T> sin(const Complex<T> &z)
// sin(-z) = -sin(z)
{  return Complex<T>((T)std::sin(z.real())*(T)::cosh(z.imag()), (T)std::cos(z.real())*(T)std::sinh(z.imag()));  }

template<typename T>
Complex<T> cos(const Complex<T> &z)
// cos(-z) = cos(z)
{  return Complex<T>((T)std::cos(z.real())*(T)::cosh(z.imag()), -(T)std::sin(z.real())*(T)std::sinh(z.imag()));  }
...
template<typename T>
Complex<T> asin(const Complex<T> &z, const int iIndex = 0)
...
template<typename T>
Complex<T> acos(const Complex<T> &z, const int iIndex = 0)
{
...
// sinh(-z) = -sinh(z)
{  return Complex<T>((T)std::sinh(z.real())*(T)std::cos(z.imag()), (T)::cosh(z.real())*(T)std::sin(z.imag()));  }

template<typename T>
Complex<T> cosh(const Complex<T> &z)
// cosh(i*z) = cos(z), cos(i*z) = cosh(z)
// cosh(-z) = cosh(z)
{  return Complex<T>((T)::cosh(z.real())*(T)std::cos(z.imag()), (T)std::sinh(z.real())*(T)std::sin(z.imag()));  }
...
template<typename T>
Complex<T> asinh(const Complex<T> &z, const int iIndex = 0)
{
...
template<typename T>
Complex<T> acosh(const Complex<T> &z, const int iIndex = 0)
{
	return ln(z + sqrt(z*z - Complex<T>(T(1)), iIndex), iIndex);
	//return ln(z - sqrt(z*z - Complex<T>(T(1)), iIndex), iIndex);
}
