...
protected:
	ContinuousNonlinearStochasticSystem(const size_t stateDim, const size_t inputDim, const size_t outputDim)
...
public:
	...
	virtual gsl_vector * evaluatePlantEquation(const double time, const gsl_vector *state) const = 0;  // f = f(t, x(t), u(t), 0)
	...
	virtual gsl_vector * evaluateMeasurementEquation(const double time, const gsl_vector *state) const = 0;  // h = h(t, x(t), u(t), 0)
