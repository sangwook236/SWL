function CPD = condMixVonMisesTiedCpdCreate_sangwook(mu, kappa, M, varargin)
%% Create a local CPD representing a tied mixture of von Mises distributions
% where the state of the hidden parent influences only the mixing weights,
% not mu and kappa. When used as the observation model for an HMM, it is
% called a semi-continuous or tied-mixture HMM.
%
%  p(Xt|St=i) = sum_m p(Mt=m|St=i) vm_pdf(Xt|mu(m), kappa(m))
%
%% Inputs
%
% mu is a matrix of size d-by-nmix 
%
% kappa is a matrix of size d-by-nmix
%
...
%	[ref] ${PMTK_HOME}/toolbox/GraphicalModels/cpd/condMixGaussTiedCpdCreate.m
...
CPD = structure(mu, kappa, M, nmix, nstates, d, prior);
CPD.cpdType    = 'condVonMisesTied';
%% 'methods'
CPD.fitFn      = @condMixVonMisesTiedCpdFit_sangwook;
CPD.fitFnEss   = @condMixVonMisesTiedCpdFitEss_sangwook;
CPD.essFn      = @condMixVonMisesTiedCpdComputeEss_sangwook;
...
%cpd.Sigma = Sigma; 
% TODO [check] >>
cpd.kappa = randn(d, nmix);
cpd.M = normalize(rand(nstates, nmix), 1);
end

function cpd = condMixVonMisesTiedCpdFit_sangwook(cpd, Z, Y) % or perhaps (cpd, Z, M, Y)?
error('Fitting a condMixVonMisesTiedCpd given fully observed data is not yet supported'); 
end

function ess = condMixVonMisesTiedCpdComputeEss_sangwook(cpd, data, gamma, B)
%% Compute the expected sufficient statistics for a condMixVonMisesTiedCpd
% data is nobs-by-d
...
function cpd = condMixVonMisesTiedCpdFitEss_sangwook(cpd, ess)
...
if ~doMap % mle
    ...
    cpd.kappa = bsxfun(@rdivide, XX, permute(Rk(:), [3, 2, 1]));
else      
    %Sigma = zeros(d, d, nmix);
    kappa = zeros(d, nmix);
    mu    = zeros(d, nmix);
