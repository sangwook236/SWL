function [model, loglikHist] = hmmFitEm_sangwook(data, nstates, type, varargin)
...
%	[ref] ${PMTK_HOME}/toolbox/LatentVariableModels/hmm/sub/hmmFitEm.m
...
%--S [] 2013/04/11: Sang-Wook Lee
    case 'vonmises'     , initFn = @initVonMises_sangwook;
    case 'mixvonmisestied'  , initFn = @initMixVonMisesTied_sangwook;
%--E [] 2013/04/11: Sang-Wook Lee
...
%--S [] 2013/04/11: Sang-Wook Lee
function model = initVonMises_sangwook(model, data, restartNum, emissionPrior)
%% Initialize the model given a von Mises emission distribution
d = size(data{1}, 1);
model.d = d;
if isempty(model.emission) || isempty(model.pi) || isempty(model.A)
    if restartNum == 1
        ...
        %Sigma = bsxfun(@plus, model.emission.Sigma, eye(model.d));
        %model.emission.Sigma = Sigma;  % regularize MLE
        model.emission.kappa = model.emission.kappa
    else 
        nstates     = model.nstates;
        stackedData = cell2mat(data')';
        mu          = zeros(d, nstates);
        ...
        for k = 1:nstates
            ...
            mu(:, k)       = colvec(mean(XX));
            %Sigma(:, :, k) = cov(XX);
            kappa(:, k) = colvec(calcKappa_sangwook(XX));
        end
        %model.emission = condGaussCpdCreate(mu, Sigma);
        model.emission = condVonMisesCpdCreate_sangwook(mu, kappa);
        model = rndInitPiA(model); 
    end
end
if ~isempty(emissionPrior)
...
function kappa = calcKappa_sangwook(XX)
    A = sqrt(mean(cos(XX)).^2 + mean(sin(XX)).^2);
	if A < -0.995
		kappa = -700;
	elseif A > 0.995
		kappa = 700;
	    ...
	    recalc_count = 1;
		while true
	        [ kappa, fval, exitflag ] = fzero(@(x) A - besseli(1, x) / besseli(0, x), kappa);

			if isnan(kappa) || kappa < -700 || kappa > 700
				kappa = -700 + (700 - -700) .* rand;
			end;
	        if 1 ~= exitflag
...
function model = initMixVonMisesTied_sangwook(model, data, restartNum, emissionPrior)
%% Initialize the model given a tied mixVonMises emission distribution
assert(~isempty(model.nmix)); % you must specify the number of mixture components
d = size(data{1}, 1);
model.d = d;
nstates     = model.nstates;
...
if isempty(model.emission) || isempty(model.pi) || isempty(model.A)
    if restartNum == 1
        ...
        nmix = model.nmix;
        %mixModel = mixModelFit(stackedData, nmix, 'gauss', 'verbose', false, 'maxIter', 10);
        mixModel = mixVonMisesFit_sangwook(stackedData, nmix, 'verbose', false, 'maxIter', 10);
        if isempty(model.emission)
            mu = mixModel.cpd.mu;
            %Sigma = bsxfun(@plus, eye(d), mixModel.cpd.Sigma);
            kappa = mixModel.cpd.kappa;
            M = repmat(mixModel.mixWeight, nstates, 1); 
            M = normalize(M + rand(size(M)), 1); %break symmetry
            %model.emission = condMixGaussTiedCpdCreate(mu, Sigma, M);
            model.emission = condMixVonMisesTiedCpdCreate_sangwook(mu, kappa, M);
        end
        model = rndInitPiA(model);
    else        
        mu          = zeros(d, nmix);
...
switch lower(model.type)
  ...
  %--S [] 2013/05/02: Sang-Wook Lee
  case 'vonmises'
    mixModel    = mixVonMisesFit_sangwook(stackedData, nstates,  'verbose', false, 'maxIter', 10);
  %--E [] 2013/05/02: Sang-Wook Lee
  otherwise
...
if isempty(model.A) || isempty(model.pi)
    ...
    switch lower(model.type)
	  ...
	  %--S [] 2013/05/11: Sang-Wook Lee
	  case 'vonmises'
        pz = mixVonMisesInferLatent_sangwook(mixModel, stackedData);
	  %--E [] 2013/05/11: Sang-Wook Lee
      otherwise
        error(['unrecognized observation distribution ' model.type])
    end
    [~,z] = max(pz,[],2);
