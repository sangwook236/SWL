...
for kk = 1:annotation_len
	...
	gesture_label_str = sprintf('g%d', gesture_label_id);
	feature_target_gesture_directory_path = strcat(feature_target_directory_path, '\', gesture_label_str);
	if ~exist(feature_target_gesture_directory_path, 'dir')
		mkdir(feature_target_directory_path, gesture_label_str);
	end;
	...
	% write feature files.
	target_feature_gesture_file_path = strcat(feature_target_gesture_directory_path, '\', feature_file_prefix, file_idx_str, '_', gesture_idx_str, '.', feature_type_name);
	dlmwrite(target_feature_gesture_file_path, size(feature_seq));
	dlmwrite(target_feature_gesture_file_path, feature_seq, '-append');
end;
