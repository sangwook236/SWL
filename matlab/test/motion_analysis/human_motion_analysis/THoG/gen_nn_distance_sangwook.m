function gen_nn_distance_sangwook(data, num_neighbors, block_size, save_type, histogram_comparison_method, distance_matrix_base_file_name)
...
%--S [] 2013/05/04: Sang-Wook Lee
% For Euclidean distance, computing data.*data outside the loop to save time
%tmp = full(sum(data.*data, 2));
tmp = zeros(n,n);
for ii = 1:n
	for jj = 1:n
		tmp(ii,jj) = compare_hist(data(ii,:), data(jj,:), histogram_comparison_method);
	end;
end;
tmp = full(tmp);
%--E [] 2013/05/04: Sang-Wook Lee
...
if (save_type == 0) || (save_type == 2)
  ...
  outfile = [distance_matrix_base_file_name, histogram_comparison_method, '_', num2str(num_neighbors), '_NN_sym_distance.mat'];
  save(outfile, 'A');
end

if (save_type == 1) || (save_type == 2)
  disp('Writing .txt file...');
  outfile = [distance_matrix_base_file_name, histogram_comparison_method, '_', num2str(num_neighbors), '_NN_sym_distance.txt'];
  fid = fopen(outfile, 'w');
  n = size(A, 1);
  for i = 1:n
    if mod(i, 10000) == 0
