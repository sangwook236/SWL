...
%line_init = [1 ; -5 ; 1];
angle_init_hat = circ_mean(angle);
...
%[line_hat, fval, exitflag, output] = fminunc(@(x) line_segment_residual(x, x1, y1, x2, y2, pi / 2), line_init, options);
%[line_hat, fval, exitflag, output] = fminunc(@(x) line_segment_residual(x, x1, y1, x2, y2, pi / 3), line_init, options);
weight_fun = inline('scale * cos(x*2) - scale + 1', 'x', 'scale');  % 0 < scale <= 0.5.
%[line_hat, fval, exitflag, output] = fminunc(@(x) line_segment_weighted_residual(x, x1, y1, x2, y2, weight_fun, pi / 2), line_init, options);
[line_hat, fval, exitflag, output] = fminunc(@(x) line_segment_weighted_residual(x, x1, y1, x2, y2, weight_fun, pi / 3), line_init, options);
