...
class ImgaugDataGenerator(Data2Generator):
	@staticmethod
	def _generateBatchesInParallelWithoutOutputAugmentation(imgaug_augmenter, inputs, outputs, batch_size, shuffle=True, *args, **kwargs):
		...
		with imgaug_augmenter.pool(processes=4) as pool:
			batch_gen = ImgaugDataGenerator._createBatchGeneratorInParallelWithoutOutputAugmentation(inputs, outputs, batch_size, shuffle)
	...
	@staticmethod
	def _createBatchGeneratorInParallelWithoutOutputAugmentation(inputs, outputs, batch_size, shuffle):
	...
	@staticmethod
	def _generateBatchesInParallelWithOutputAugmentation(imgaug_augmenter, inputs, outputs, batch_size, shuffle=True, *args, **kwargs):
		#if not isinstance(imgaug_augmenter, iaa.Sequential):
		#	raise ValueError('The augmenter has to be an instance of imgaug.augmenters.Sequential to augment in parallel')
		...
		#with imgaug_augmenter.pool(processes=-1, maxtasksperchild=20, seed=123) as pool:
		with imgaug_augmenter.pool(processes=4) as pool:
			batch_gen = ImgaugDataGenerator._createBatchGeneratorInParallelWithOutputAugmentation(inputs, outputs, batch_size, shuffle)
	...
	@staticmethod
	def _createBatchGeneratorInParallelWithOutputAugmentation(inputs, outputs, batch_size, shuffle):
		num_examples = len(inputs)
		if batch_size is None:
			batch_size = num_examples
		if batch_size <= 0:
