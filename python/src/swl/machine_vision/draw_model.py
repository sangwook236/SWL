...
class DRAW(object):
	...
	def create_training_model(self):
		self._model_output, mus, logsigmas, sigmas = self._create_single_model(self._input_tensor_ph, self._input_shape, True)
	...
	def _create_single_model(self, input_tensor, input_shape, is_training):
		...
		self._reuse = tf.AUTO_REUSE  # Workaround for variable_scope(reuse=True).
		...
		canvases = [0] * self._num_time_steps  # Sequence of canvases.
		mus, logsigmas, sigmas = [0] * self._num_time_steps, [0] * self._num_time_steps, [0] * self._num_time_steps  # Gaussian params generated by SampleQ. We will need these for computing loss.
	...
	def _get_loss(self, canvases, input, mus, logsigmas, sigmas):
