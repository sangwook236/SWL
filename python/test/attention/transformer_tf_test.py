...
def evaluate(inp_sentence, transformer, tokenizer_en, tokenizer_pt, max_length):
...
def translate(sentence, transformer, tokenizer_en, tokenizer_pt, max_length, plot=''):
	result, attention_weights = evaluate(sentence, transformer, tokenizer_en, tokenizer_pt, max_length)
...
def positional_encoding_test():
	d_model, num_terms = 512, 50
	pos_encoding = positional_encoding(num_terms, d_model)
...
def masking_test():
	...
	print('create_padding_mask() =', create_padding_mask(x))

	x = tf.random.uniform((1, 3))
	print('create_look_ahead_mask() =', create_look_ahead_mask(x.shape[1]))
...
def point_wise_feed_forward_network_test():
	sample_ffn = point_wise_feed_forward_network(512, 2048)
	print(sample_ffn(tf.random.uniform((64, 50, 512))).shape)

def encoder_and_decoder_layer_test():
...
def custom_schedule_test():
	d_model = 128
	temp_learning_rate_schedule = CustomSchedule(d_model)

	lr_schedule = temp_learning_rate_schedule(tf.range(40000, dtype=tf.float32))
...
# REF [site] >> https://www.tensorflow.org/tutorials/text/transformer
def transformer_tutorial():
	...
	translate('este é um problema que temos que resolver.', transformer, tokenizer_en, tokenizer_pt, MAX_LENGTH)
	print('Real translation: this is a problem we have to solve .')

	translate('os meus vizinhos ouviram sobre esta ideia.', transformer, tokenizer_en, tokenizer_pt, MAX_LENGTH)
	print('Real translation: and my neighboring homes heard about this idea .')

	translate('vou então muito rapidamente partilhar convosco algumas histórias de algumas coisas mágicas que aconteceram.', transformer, tokenizer_en, tokenizer_pt, MAX_LENGTH)
	print("Real translation: so i 'll just share with you some stories very quickly of some magical things that have happened .")

	translate('este é o primeiro livro que eu fiz.', transformer, tokenizer_en, tokenizer_pt, MAX_LENGTH, plot='decoder_layer4_block2')
...
def main():
	...
	#point_wise_feed_forward_network_test()
	#encoder_and_decoder_layer_test()
	#encoder_and_decoder_test()
	#transformer_test()
	#custom_schedule_test()
