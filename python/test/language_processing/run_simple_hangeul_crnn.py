...
class MyModel(object):
	...
	def _get_loss(self, y, t_sparse, y_len, t_len, is_time_major):
		...
		#loss = tf.nn.ctc_loss_v2(labels=t_sparse, logits=y, label_length=self._output_len_ph, logit_length=y_len, logits_time_major=is_time_major, unique=None, blank_index=None)
		#loss = tf.nn.ctc_loss_v2(labels=t, logits=y, label_length=self._output_len_ph, logit_length=y_len, logits_time_major=is_time_major, unique=None, blank_index=None)
...
class MyRunner(object):
	def __init__(self, is_dataset_generated_at_runtime, data_dir_path=None, train_test_ratio=0.8):
		...
		if is_dataset_generated_at_runtime:
			...
			start_time = time.time()
			self._dataset = text_line_data.RunTimeTextLineDataset(set(korean_words), image_height, image_width, image_channel, max_label_len=max_label_len)
