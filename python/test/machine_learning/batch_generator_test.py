...
def simple_npy_file_batch_generator_example():
	...
	for epoch in range(num_epochs):
		...
		npy_filepath_pairs = batchGenerator.saveBatches(dir_path)  # Generates and saves batches.

		batchLoader = NpyFileBatchLoader()
		batches = batchLoader.loadBatches(npy_filepath_pairs)  # Loads batches.
		for idx, batch in enumerate(batches):
			# Can run in an individual thread or process.
			# Augment each batch (images & labels).
			# Train with each batch (images & labels).
