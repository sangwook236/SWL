...
class FigureLabelMeDataset(torch.utils.data.Dataset):
	...
	def _load_from_json_async(self, json_filepaths, image_channel, is_preloaded_image_used):
		...
		#with mp.Pool(processes=num_processes, initializer=initialize_lock, initargs=(lock,)) as pool:
		with mp.Pool(processes=num_processes) as pool:
			#results = pool.map_async(functools.partial(self._worker_proc, valid_labels=self.valid_labels, flag=flag, is_preloaded_image_used=is_preloaded_image_used), json_filepaths)
			results = pool.map_async(functools.partial(self._worker_proc, valid_labels=self.valid_labels, flag=flag, is_preloaded_image_used=is_preloaded_image_used), json_filepaths, callback=async_callback)
...
class FigureDetectionLabelMeDataset(torch.utils.data.Dataset):
	def __init__(self, json_filepaths, image_channel, is_preloaded_image_used=True, transform=None):
		...
		#self.figures = self._load_from_json(json_filepaths, image_channel, is_preloaded_image_used)
		self.figures = self._load_from_json_async(json_filepaths, image_channel, is_preloaded_image_used)
		...
		for json_filepath in json_filepaths:
			...
			try:
				...
				for shape in json_data['shapes']:
					...
					labels.append(self.valid_labels.index(label))
				figures.append([img if is_preloaded_image_used else image_filepath, np.array(bboxes), np.array(labels)] if bboxes and labels else None)
			except Exception as ex:
