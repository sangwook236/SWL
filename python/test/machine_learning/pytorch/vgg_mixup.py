...
class VGG(nn.Module):
	...
	def __init__(self, cfg, batch_norm=False, num_classes=1000, init_weights=True):
		...
		in_channels = 3
		self.layer1, in_channels = self._make_layers(cfg[0], in_channels, batch_norm)
		self.layer2, in_channels = self._make_layers(cfg[1], in_channels, batch_norm)
		self.layer3, in_channels = self._make_layers(cfg[2], in_channels, batch_norm)
		self.layer4, in_channels = self._make_layers(cfg[3], in_channels, batch_norm)
		self.layer5, in_channels = self._make_layers(cfg[4], in_channels, batch_norm)
...
def _vgg(arch, cfg, batch_norm, pretrained, progress, **kwargs):
	...
	model = VGG(cfgs[cfg], batch_norm=batch_norm, **kwargs)
	if pretrained:
