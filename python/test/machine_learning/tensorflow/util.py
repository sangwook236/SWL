...
def evaluate_neural_net(session, nnEvaluator, val_inputs, val_outputs, batch_size, saver=None, checkpoint_dir_path=None, is_time_major=False, is_sparse_label=False):
	...
	if num_val_examples > 0:
		...
		val_loss, val_acc = nnEvaluator.evaluate(session, val_inputs, val_outputs, num_val_examples if is_sparse_label else batch_size)
		print('\tEvaluation time = {}'.format(time.time() - start_time))
		print('\tValidation loss = {}, validation accurary = {}'.format(val_loss, val_acc))
		print('[SWL] Info: End evaluation...')
	else:
...
def evaluate_neural_net_with_decoder_input(session, nnEvaluator, val_encoder_input_seqs, val_decoder_input_seqs, val_decoder_output_seqs, batch_size, saver=None, checkpoint_dir_path=None, is_time_major=False, is_sparse_label=False):
	batch_axis = 1 if is_time_major else 0
	...
	if num_val_examples > 0:
		...
		val_loss, val_acc = nnEvaluator.evaluate_seq2seq(session, val_encoder_input_seqs, val_decoder_input_seqs, val_decoder_output_seqs, num_val_examples if is_sparse_label else batch_size)
		print('\tEvaluation time = {}'.format(time.time() - start_time))
...
# Supports dense or sparse labels.
# But when labels are sparse, all dataset is processed at once.
def infer_by_neural_net(session, nnInferrer, test_inputs, batch_size, saver=None, checkpoint_dir_path=None, is_time_major=False, is_sparse_label=False):
	batch_axis = 1 if is_time_major else 0
	...
	if num_inf_examples > 0:
		...
		inferences = nnInferrer.infer(session, test_inputs, num_inf_examples if is_sparse_label else batch_size)
		print('\tInference time = {}'.format(time.time() - start_time))
		print('[SWL] Info: End inferring...')
...
def infer_by_neural_net_with_decoder_input(session, nnInferrer, dataset, test_inputs, batch_size, saver=None, checkpoint_dir_path=None, is_time_major=False, is_sparse_label=False):
	return infer_by_neural_net(session, nnInferrer, test_inputs, batch_size, saver, checkpoint_dir_path, is_time_major, is_sparse_label):
