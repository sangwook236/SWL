	...
	for (size_t n = N - 1; n > 0; --n)
	{
		for (k = 0; k < K_; ++k)
		{
			...
			for (i = 0; i < K_; ++i)
				//sum += A_[k][i] * B_[i][observations[n]] * beta[n][i];
				sum += A_[k][i] * evaluateEmissionProbability(i, observations[boost::indices[n][boost::multi_array<double, 2>::index_range()]]) * beta[n][i];
			beta[n-1][k] = sum / scale[n];
		}
	}
}

void CDHMM::runViterbiAlgorithm(const size_t N, const boost::multi_array<double, 2> &observations, boost::multi_array<double, 2> &delta, boost::multi_array<int, 2> &psi, std::vector<int> &states, double &probability, const bool useLog /*= true*/) const
{
...
void CDHMM::runViterbiAlgorithmNotUsigLog(const size_t N, const boost::multi_array<double, 2> &observations, boost::multi_array<double, 2> &delta, boost::multi_array<int, 2> &psi, std::vector<int> &states, double &probability) const
{
	size_t i, k;  // state indices
	size_t n;  // time index
...
void CDHMM::runViterbiAlgorithmUsingLog(const size_t N, const boost::multi_array<double, 2> &observations, boost::multi_array<double, 2> &delta, boost::multi_array<int, 2> &psi, std::vector<int> &states, double &probability) const
{
	size_t i, k;  // state indices
	size_t n;  // time index
...
void CDHMM::generateSample(const size_t N, boost::multi_array<double, 2> &observations, std::vector<int> &states) const
{
	// PRECONDITIONS [] >>
	//	-. std::srand() had to be called before this function is called.

	states[0] = generateInitialState();
	generateObservationsSymbol(states[0], observations[boost::indices[0][boost::multi_array<double, 2>::index_range()]], true);

	for (size_t n = 1; n < N; ++n)
	{
		states[n] = generateNextState(states[n-1]);
		generateObservationsSymbol(states[n], observations[boost::indices[n][boost::multi_array<double, 2>::index_range()]]);
	}
}
