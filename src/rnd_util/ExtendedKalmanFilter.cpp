...
bool ExtendedKalmanFilter::propagate(const double time)
{
	...
	if (!A || !C || !W || !V || !Qd || !Rd || !f_eval || !h_eval || !y_tilde) return false;

	//-------------------------------------------------------------------------
	// continuous extended Kalman filter time update equations (prediction)
	...
	//-------------------------------------------------------------------------
	// continuous extended Kalman filter measurement update equations (correction)
...
bool ExtendedKalmanFilter::propagate(const size_t step)  // 1-based time step. 0-th time step is initial
{
	...
	if (!Phi || !Cd || !Qd || !Rd || !f_eval || !h_eval || !y_tilde) return false;

	//-------------------------------------------------------------------------
	// 1. time update (prediction)

	// FIXME [modify] >> some quantities at the time step, k-1 are required
	//	Phi(k-1), Q(k-1) & u(k-1) are used to predict x-(k)

	// (1) propagate time
	// x-(k) = f(k-1, x(k-1), u(k-1), 0)
	gsl_vector_memcpy(x_hat_, f_eval);

	// P-(k) = Phi(k-1) * P(k-1) * Phi(k-1)^T + Qd(k-1) where Phi(k-1) = df(k-1, x(k-1), u(k-1), 0)/dx, Qd(k-1) = W(k-1) * Q(k-1) * W(k-1)^T, W(k-1) = df(k-1, x(k-1), u(k-1), 0)/dw
#if 0
	// using Q
...
bool ExtendedKalmanFilter::propagate(const size_t step)  // 0-based time step. 0-th time step is initial
{
	//-------------------------------------------------------------------------
	// the discrete plant:
	//
